...
exports.IndicatorsSync = void 0;
const ti_buffer_1 = require("./ti_buffer");
class IndicatorsSync {
    constructor() { }
    // ################## Utils
    /**
     ...
     * @returns
     */
    normalize(originalLength, source, empty = NaN) {
     ...
     * @returns
     */
    ad(high, low, close, volume, size = close.length) {
        const output = [];
        let sum = 0;
     ...
     * @returns
     */
    adosc(high, low, close, volume, short_period, long_period, size = close.length) {
     ...
     * @returns
     */
    adx(high, low, period, size = high.length) {
        const output = [];
     ...
     * @returns
     */
    adxr(high, low, period, size = high.length) {
        const output = [];
        // if (period < 2) return "Invalid Options"
     ...
     * @returns
     */
    apo(source, short_period, long_period, size = source.length) {
        const output = [];
        // if (short_period < 1) return "Invalid Options"
     ...
     */
    aroon(high, low, period, size = high.length) {
        const adown = [];
        const aup = [];
     ...
     * @returns
     */
    aroonosc(high, low, period, size = high.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period) return "Out of range"
        const scale = 100.0 / period;
     ...
     * @returns
     */
    atr(high, low, close, period, size = high.length) {
        const output = [];
        // if (period < 1) return "Invalid options"
     ...
     * @returns
     */
    avgprice(open, high, low, close, size = open.length) {
        const output = [];
        for (let index = 0; index < size; ++index) {
     ...
     */
    bbands(source, period, stddev, size = source.length) {
        const lower = [];
        const middle = [];
        const upper = [];
     ...
     * @returns
     */
    bop(open, high, low, close, size = open.length) {
        const output = [];
        for (let index = 0; index < size; ++index) {
            const hl = high[index] - low[index];
            if (hl <= 0.0) {
     ...
     * @returns
     */
    cci(high, low, close, period, size = high.length) {
        const scale = 1.0 / period;
        // if (period < 1) return "Invalid Options"
        // if (size <= (period-1) * 2) return "Out of range"
        const output = [];
        const sum = new ti_buffer_1.ti_buffer(period);
        let i, j;
        for (i = 0; i < size; ++i) {
            // Start TYPEPRICE
     ...
     * @returns
     */
    cmo(source, period, size = source.length) {
        const output = [];
        // if (period < 1) return "Invalid options"
        // if (size <= period) return "Out of range"
        let up_sum = 0;
     ...
     * @returns
     */
    crossOverNumber(seriesA, number) {
        const output = [];
     ...
     * @returns
     */
    crossUnderNumber(seriesA, number) {
        const output = [];
        for (let index = 0; index < seriesA.length; index++) {
            const current = seriesA[index];
            const pre = seriesA[index - 1];
     ...
     * @returns
     */
    cvi(high, low, period, size = high.length) {
        const output = [];
        const per = 2 / (period + 1);
        const lag = new ti_buffer_1.ti_buffer(period);
     ...
     * @returns
     */
    _cvi(high, low, period, size = high.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
     ...
     * @returns
     */
    decay(source, period, size = source.length) {
        const output = [];
        const scale = 1.0 / period;
     ...
     * @returns
     */
    dema(source, period, size = source.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= (period-1) * 2) return "Out of range"
        const per = 2 / (period + 1);
     ...
     */
    di(high, low, close, period, size = high.length) {
        const plus_di = [];
        const minus_di = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
     ...
     */
    dm(high, low, period, size = high.length) {
        const plus_dm = [];
        const minus_dm = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
     ...
     * @returns
     */
    dpo(source, period, size = source.length) {
     ...
     * @returns
     */
    dx(high, low, period, size = high.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
        const per = (period - 1) / (period);
     ...
     * @returns
     */
    edecay(source, period, size = source.length) {
        const output = [];
     ...
     * @returns
     */
    ema(source, period, size = source.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
     ...
     * @returns
     */
    emv(high, low, volume, size = high.length) {
        const output = [];
        // if (size <= 1) return "Out of range"
     ...
     */
    fisher(high, low, period, size = high.length) {
        const fisher = [];
        const signal = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
     ...
     * @returns
     */
    fosc(source, period, size = source.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period) return "Out of range"
     ...
     * @deprecated
     */
    DEP_hma(input, period, size = input.length) {
        const output = [];
     ...
     * @returns
     */
    hma(input, period, size = input.length) {
        const output = [];
        const period2 = Math.floor(period / 2);
        const periodsqrt = Math.floor(Math.sqrt(period));
        const weights = period * (period + 1) / 2;
        ...
        const buff = new ti_buffer_1.ti_buffer(periodsqrt);
        for (i = period - 1; i < size; ++i) {
            weight_sum += input[i] * period;
            sum += input[i];
            weight_sum2 += input[i] * period2;
     ...
     * @returns
     */
    kama(source, period, size = source.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
     ...
     * @returns
     */
    kvo(high, low, close, volume, short_period, long_period, size = high.length) {
        // if (short_period < 1) return "Invalid Options"
        // if (long_period < short_period) return "Invalid Options"
        // if (size <= 1) return "Out of range"
        const short_per = 2 / (short_period + 1);
     ...
     * @returns
     */
    lag(source, period, size = source.length) {
        const output = [];
     ...
     * @returns
     */
    linreg(source, period, size = source.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
        // Start LINEAR_REGRESSION
     ...
     * @returns
     */
    linregintercept(source, period, size = source.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
        // Start LINEAR_REGRESSION
     ...
     * @returns
     */
    linregslope(source, period, size = source.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
        // Start LINEAR_REGRESSION
     ...
     */
    macd(source, short_period, long_period, signal_period, size = source.length) {
        const macd = [];
        const signal = [];
        const hist = [];
        // if (short_period < 1) return "Invalid Options"
     ...
     * @returns
     */
    marketfi(high, low, volume, size = high.length) {
        const output = [];
        // if (size <= 0) return "Out of range"
        for (let i = 0; i < size; ++i) {
     ...
     * @returns
     */
    mass(high, low, period, size = high.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
     ...
     * @returns
     */
    max(source, period, size = source.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
        let trail = 0;
     ...
     * @returns
     */
    md(source, period, size = source.length) {
        const output = [];
        const scale = 1.0 / period;
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
     ...
     * @returns
     */
    medprice(high, low, size = high.length) {
        const output = [];
        for (let i = 0; i < size; ++i) {
     ...
     * @returns
     */
    mfi(high, low, close, volume, period, size = high.length) {
        // if (period < 1) return "Invalid Options"
        // if (size <= period) return "Out of range"
        const output = [];
        // Start TYPPRICE()
     ...
     * @returns
     */
    min(source, period, size = source.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
        let trail = 0;
     ...
     * @returns
     */
    mom(source, period, size = source.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period) return "Out of range"
        for (let i = period; i < size; ++i) {
     ...
     * @returns
     */
    msw(source, period, size = source.length) {
        const sine = [];
        const lead = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period) return "Out of range"
     ...
     * @returns
     */
    natr(high, low, close, period, size = high.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
        const per = 1.0 / (period);
     ...
     * @returns
     */
    nvi(close, volume, size = close.length) {
        const output = [];
        // if (size <= 0) return "Out of range"
        let nvi = 1000;
        output.push(nvi);
     ...
     * @returns
     */
    obv(close, volume, size = close.length) {
        const output = [];
        let sum = 0;
        output.push(sum);
        let prev = close[0];
     ...
     * @returns
     */
    ppo(source, short_period, long_period, size = source.length) {
        const ppo = [];
        // if (short_period < 1) return "Invalid Options"
        // if (long_period < 2) return "Invalid Options"
        // if (long_period < short_period) return "Invalid Options"
     ...
     * @returns
     */
    psar(high, low, accel_step, accel_max, size = high.length) {
        const output = [];
     ...
     * @returns
     */
    pvi(close, volume, size = close.length) {
        const output = [];
        // if (size <= 0) return "Out of range"
        let pvi = 1000;
        output.push(pvi);
     ...
     * @returns
     */
    qstick(open, close, period, size = close.length) {
        const output = [];
        const scale = 1.0 / period;
        let sum = 0;
        let i;
     ...
     * @returns
     */
    roc(source, period, size = source.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period) return "Out of range"
        for (let i = period; i < size; ++i) {
     ...
     * @returns
     */
    rocr(source, period, size = source.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period) return "Out of range"
        for (let i = period; i < size; ++i) {
     ...
     * @returns
     */
    rsi(source, period, size = source.length) {
        const output = [];
        const per = 1.0 / (period);
        // if (period < 1) return "Invalid Options"
        // if (size <= period) return "Out of range"
     ...
     * @returns
     */
    sma(source, period, size = source.length) {
        const output = [];
        const scale = 1.0 / period;
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
     ...
     * @returns
     */
    stddev(source, period, size = source.length) {
        const output = [];
        const scale = 1.0 / period;
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
     ...
     * @returns
     */
    stderr(source, period, size = source.length) {
        const output = [];
        const scale = 1.0 / period;
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
     ...
     */
    stoch(high, low, close, kperiod, kslow, dperiod, size = high.length) {
     ...
     * @returns
     */
    stochrsi(source, period, size = source.length) {
        const output = [];
        const per = 1.0 / (period);
        // if (period < 2) return "Invalid Options"
        // if (size <= (period) * 2 - 1) return "Out of range"
     ...
     * @returns
     */
    sum(source, period, size = source.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
        let sum = 0;
     ...
     * @returns
     */
    tema(source, period, size = source.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= (period - 1) * 3) return "Out of range"
        const per = 2 / (period + 1);
     ...
     * @returns
     */
    tr(high, low, close, size = high.length) {
        const output = [];
        let truerange;
     ...
     * @returns
     */
    trima(source, period, size = source.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
     ...
     * @returns
     */
    trix(source, period, size = source.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
     ...
     * @returns
     */
    tsf(source, period, size = source.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
        // Start LINEAR_REGRESSION
     ...
     * @param size
     */
    typprice(high, low, close, size = high.length) {
        const output = [];
        for (let i = 0; i < size; ++i) {
     ...
     * @returns
     */
    ultosc(high, low, close, short_period, medium_period, long_period, size = high.length) {
        const output = [];
        // if (short_period < 1) return "Invalid Options"
     ...
     * @returns
     */
    var(source, period, size = source.length) {
        const output = [];
        const scale = 1.0 / period;
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
     ...
     * @returns
     */
    vhf(source, period, size = source.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period) return "Out of range"
        let trail = 1;
     ...
     * @returns
     */
    vidya(source, short_period, long_period, alpha, size = source.length) {
        const output = [];
        ...
        if (long_period - 1 < size) {
            ...
            if (k != k)
                k = 0; /* In some conditions it works out that we take the sqrt(-0.0), which gives NaN.
     ...
     * @returns
     */
    volatility(source, period, size = source.length) {
        const output = [];
        const scale = 1.0 / period;
     ...
     * @returns
     */
    vosc(source, short_period, long_period, size = source.length) {
        const output = [];
        const short_div = 1.0 / short_period;
        const long_div = 1.0 / long_period;
        // if (short_period < 1) return "Invalid Options"
     ...
     * @returns
     */
    vwma(source, volume, period, size = source.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
        let sum = 0;
     ...
     * @returns
     */
    wad(high, low, close, size = high.length) {
        // if (size <= 1) return "Out of range"
        const output = [];
        let sum = 0;
        let yc = close[0];
     ...
     * @param size
     */
    wcprice(high, low, close, size = high.length) {
        const output = [];
        for (let i = 0; i < size; ++i) {
     ...
     * @returns
     */
    wilders(source, period, size = source.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
        const per = 1.0 / (period);
     ...
     * @returns
     */
    willr(high, low, close, period, size = high.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
        let trail = 0;
     ...
     * @returns
     */
    wma(source, period, size = source.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
        const weights = period * (period + 1) / 2;
     ...
     * @returns
     */
    zlema(input, period, size = input.length) {
     ...
     */
    abands(high, low, close, period, size = high.length) {
        const upper_band = [];
        const lower_band = [];
        const middle_point = [];
        // if (period < 1) return "Invalid Options"
     ...
     * @returns
     */
    alma(source, period, offset, sigma, size = source.length) {
        const output = [];
        // validate options
        // if (period < 1) return "Invalid Options"
     ...
     */
    ce(high, low, close, period, coef, size = high.length) {
        const ce_high = [];
        const ce_low = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
     ...
     * @returns
     */
    cmf(high, low, close, volume, period, size = high.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
     ...
     * @returns
     */
    fi(close, volume, period, size = close.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= 1) return "Out of range"
        const per = 2. / (period + 1.);
     ...
     */
    kc(high, low, close, period, multiple, size = high.length) {
        const kc_lower = [];
        const kc_middle = [];
        const kc_upper = [];
        // if (period < 1) return "Invalid Options"
     ...
     * @returns
     */
    kst(source, roc1, roc2, roc3, roc4, ma1, ma2, ma3, ma4, size = source.length) {
        const kst = [];
        const kst_signal = [];
     ...
     */
    pbands(high, low, close, period, size = high.length) {
        const pbands_lower = [];
        const pbands_upper = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
     ...
     * @returns
     */
    pfe(source, period, ema_period, size = source.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period) return "Out of range"
     ...
     * @returns
     */
    posc(high, low, close, period, ema_period, size = high.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
     ...
     * @returns
     */
    rmi(source, period, lookback_period, size = source.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
     ...
     * @returns
     */
    rmta(source, period, beta, size = source.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
        // if (size <= period - 1) return "Out of range"
        const alpha = 1 - beta;
     ...
     * @returns
     */
    rvi(source, sma_period, stddev_period, size = source.length) {
        const output = [];
     ...
     * @returns
     */
    smi(high, low, close, q_period, r_period, s_period, size = high.length) {
        const output = [];
     ...
     * @returns
     */
    tsi(source, y_period, z_period, size = source.length) {
        const output = [];
     ...
     * @returns
     */
    vwap(high, low, close, volume, period, size = high.length) {
        const output = [];
        // if (period < 1) return "Invalid Options"
...
exports.IndicatorsSync = IndicatorsSync;
